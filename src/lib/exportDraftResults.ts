...
export function exportDraftResults(league: LeagueFullPublic): void {
  ...
  rosterSheet['!cols'] = [{ wch: 25 }, { wch: 8 }, { wch: 10 }]
  XLSX.utils.book_append_sheet(workbook, rosterSheet, 'Team Rosters')
  ...
  historySheet['!cols'] = [{ wch: 6 }, { wch: 7 }, { wch: 20 }, { wch: 20 }, { wch: 8 }, { wch: 10 }]
  XLSX.utils.book_append_sheet(workbook, historySheet, 'Pick History')
  ...
  XLSX.writeFile(workbook, `${safeName}-draft-results.xlsx`)
}
